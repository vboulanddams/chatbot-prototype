...
## Multilingue et voix (navigateur, gratuit)

**Multilingue à l'écrit :** le modèle détecte automatiquement la langue utilisée par l'utilisateur (français, anglais, etc.) et répond dans cette même langue, en traduisant à la volée le contenu de vos scripts (qui restent en français dans `knowledge_base.json` — aucune traduction manuelle à faire).

**Voix (micro + lecture des réponses) :** utilise les fonctions vocales **intégrées au navigateur** (Chrome/Edge), sans aucun service payant ni clé API supplémentaire :
- Bouton micro 🎤 à côté de la zone de texte : parlez, votre message est transcrit et envoyé automatiquement.
- Bouton 🔇/🔊 en haut à droite : active ou désactive la lecture à voix haute des réponses du chatbot.

⚠️ Limites de cette approche "navigateur" (compromis fait pour rester gratuit et simple) :
- Fonctionne sur Chrome et Edge ; le bouton micro se masque automatiquement sur les navigateurs non compatibles (ex. Firefox).
- La voix de synthèse est celle proposée par défaut par le navigateur/l'ordinateur de l'utilisateur (qualité correcte mais pas personnalisable facilement — pas de voix "clonée" avec cette méthode).
- Pas utilisable telle quelle pour un vrai canal téléphonique (répondeur/IVR) : ceci fonctionne uniquement dans le navigateur web. Pour un canal téléphonique, il faudra une solution serveur dédiée (Twilio + STT/TTS payants), qu'on pourra mettre en place plus tard si besoin.
